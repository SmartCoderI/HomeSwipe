 ...
 * Returns mock data directly without calling external APIs
 * Used for testing and development when USE_MOCK_DATA is enabled
 */

// Mock homes data (same structure as frontend/mockData.ts)
const mockHomes = [
  {
    id: "1",
  ...
  },
  {
    ...
    analysis: {
      ...
      commute: "Driving-based commute via Capitol Expressway and Highway 101 to other parts of San Jose",
  ...
  },
  {
    ...
    analysis: {
      ...
      commute: "Centrally positioned for Cupertino, Santa Clara, and North San Jose tech corridors",
...
/**
 * GET /api/mock-listings
 * Returns mock home listings for development/testing
 */
export const getMockListings = async (req, res) => {
  try {
    ...
    console.log(`[Mock Listings] Returning ${mockHomes.length} mock homes for query: "${query || 'default'}"`);

    return res.json({
      success: true,
      count: mockHomes.length,
      listings: mockHomes,
      preferences: {
        location: 'San Jose, CA',
        originalQuery: query || 'mock search'
      },
      isMock: true
    });
  } catch (error) {
    console.error('Mock listings error:', error);
    return res.status(500).json({
      error: 'Failed to get mock listings',
      message: error.message
    });
  }
};

export default { getMockListings };
